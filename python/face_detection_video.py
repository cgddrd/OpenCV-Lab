...
# Obtain video capture from the default device (0) - In this case, it is the webcam.
video_capture = cv2.VideoCapture(0)

# We will capture/process continuously...
while True:

	# Capture frame-by-frame.
	# 'ret' = return code (not used in this script)
	# 'frame' = the actual frame from the video feed.
	ret, frame = video_capture.read()
	
	# Generate a grey-scale version of the current frame (required for Haar cascade feature analysis)
	...
	for (x, y, w, h) in faces:
		...
		# smiles = smile_cascade.detectMultiScale(roi_gray, scaleFactor = 5)
			
		# for (sx, sy, sw, sh) in smiles:
			# cv2.rectangle(roi_color,(sx,sy),(sx+sw,sy+sh),(0,0,255),2)
	
	# Display the results (as a live feed) in a window.
	cv2.imshow('Facial Features Detection', frame)
	
	# If the user presses the 'q' key, abort.
